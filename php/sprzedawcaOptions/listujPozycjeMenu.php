...
$sql = "SELECT * FROM pozycje;";
try {
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $_SESSION["returnMessageString"] = $_SESSION["returnMessageString"] . "id: " . $row["id"] . " przedmiot: " . $row["przedmiot"] . " ilosc: " . $row["ilosc"] . " rabat:" . $row["rabat"] . "<br>";
        }
        $log = "Wypisano pozycje";
